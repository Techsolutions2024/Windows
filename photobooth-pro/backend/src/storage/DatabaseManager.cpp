...
bool DatabaseManager::createTables() {
    ...
    if (!executeSQL(eventsSql)) return false;
    if (!executeSQL(eventConfigSql)) return false;
    if (!executeSQL(photosSql)) return false;
    if (!executeSQL(indexSql)) return false;
...
int DatabaseManager::createEvent(const std::string& name) {
    ...
    sql << "INSERT INTO events (name, created_at, updated_at, status) VALUES ('"
        << escapeString(name) << "', '"
        << timestamp << "', '"
        << timestamp << "', 'active');";
    ...
    int eventId = static_cast<int>(sqlite3_last_insert_rowid(static_cast<sqlite3*>(db_)));
...
bool DatabaseManager::updateEvent(int eventId, const std::string& name) {
    std::string timestamp = getCurrentTimestamp();
    std::ostringstream sql;
    sql << "UPDATE events SET name = '" << escapeString(name) 
...
std::vector<Event> DatabaseManager::getAllEvents(const std::string& sortBy, 
    ...
    sql << "SELECT e.id, e.name, e.created_at, e.updated_at, e.photo_count, "
        << "e.thumbnail_path, e.status FROM events e";
    ...
    else if (sortBy == "photoCount") sortColumn = "photo_count";
    else if (sortBy == "updatedAt") sortColumn = "updated_at";
    
    sql << " ORDER BY e." << sortColumn << " " << (order == "ASC" ? "ASC" : "DESC") << ";";

    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(static_cast<sqlite3*>(db_), sql.str().c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        event.createdAt = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
        event.updatedAt = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));
        event.photoCount = sqlite3_column_int(stmt, 4);
        
        const char* thumbnail = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5));
...
std::optional<Event> DatabaseManager::getEvent(int eventId) {
    std::ostringstream sql;
    sql << "SELECT id, name, created_at, updated_at, photo_count, thumbnail_path, status "
...
bool DatabaseManager::saveEventConfig(const EventConfig& config) {
    ...
    sql << "INSERT OR REPLACE INTO event_configs ("
        << "event_id, start_screen_image, capture_mode, photo_enabled, gif_enabled, "
        ...
        << config.eventId << ", '"
        << escapeString(config.startScreenImage) << "', '"
        << escapeString(config.captureMode) << "', "
        << (config.photoEnabled ? 1 : 0) << ", "
        << (config.gifEnabled ? 1 : 0) << ", "
...
std::optional<EventConfig> DatabaseManager::getEventConfig(int eventId) {
    std::ostringstream sql;
    sql << "SELECT event_id, start_screen_image, capture_mode, photo_enabled, gif_enabled, "
        << "boomerang_enabled, video_enabled, effects_config, props_config, "
        << "beauty_filter_config, watermark_config, post_process_config, "
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        const char* startScreen = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
        config.startScreenImage = startScreen ? startScreen : "";
        
        const char* captureMode = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
        ...
        const char* effects = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7));
        config.effectsConfig = effects ? effects : "";
        
        const char* props = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 8));
        config.propsConfig = props ? props : "";
        
        const char* beauty = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 9));
        config.beautyFilterConfig = beauty ? beauty : "";
        
        const char* watermark = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 10));
        config.watermarkConfig = watermark ? watermark : "";
        
        const char* postProcess = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 11));
        ...
        const char* layout = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 14));
...
bool DatabaseManager::updateEventConfig(int eventId, const EventConfig& config) {
...
int DatabaseManager::savePhoto(const Photo& photo) {
    std::ostringstream sql;
    sql << "INSERT INTO photos (event_id, file_path, thumbnail_path, capture_mode, "
        << "timestamp, width, height, printed, shared, filter_applied, metadata) VALUES ("
        << photo.eventId << ", '"
        << escapeString(photo.filePath) << "', '"
        << escapeString(photo.thumbnailPath) << "', '"
        << escapeString(photo.captureMode) << "', '"
        << escapeString(photo.timestamp) << "', "
        << photo.width << ", "
        << photo.height << ", "
        << (photo.printed ? 1 : 0) << ", "
        << (photo.shared ? 1 : 0) << ", '"
        << escapeString(photo.filterApplied) << "', '"
    ...
    int photoId = static_cast<int>(sqlite3_last_insert_rowid(static_cast<sqlite3*>(db_)));
...
std::vector<Photo> DatabaseManager::getPhotosForEvent(int eventId, int limit, int offset) {
    std::vector<Photo> photos;
    
    std::ostringstream sql;
    sql << "SELECT id, event_id, file_path, thumbnail_path, capture_mode, timestamp, "
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        photo.filePath = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
        
        const char* thumbnail = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));
        photo.thumbnailPath = thumbnail ? thumbnail : "";
        
        const char* captureMode = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4));
        photo.captureMode = captureMode ? captureMode : "";
        
        photo.timestamp = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5));
        ...
        const char* filter = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 10));
        photo.filterApplied = filter ? filter : "";
        
        const char* metadata = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 11));
...
bool DatabaseManager::updatePhotoThumbnail(int photoId, const std::string& thumbnailPath) {
    std::ostringstream sql;
